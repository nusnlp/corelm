...
#TODO: Make validation source tex, target text and aligment file a single unit
parser.add_argument("-ts", "--tune-source-text", dest="src_tune_path",  help="Path to the source langauge tuning text file")
parser.add_argument("-tt", "--tune-target-text", dest="trg_tune_path",  help="Path to the target language tuning text file")
parser.add_argument("-ta", "--tune-alignment-file", dest="alignment_tune_path", help="Path to the alignment file for tuning text")
...
parser.add_argument("-vs","--prune-source-vocab", dest="src_vocab_size", default=16000, type=int, help="Source Vocabulary size")
parser.add_argument("-vt","--prune-target-vocab", dest="trg_vocab_size", default=16000, type=int, help="Target Vocabulary size")
parser.add_argument("-vo","--prune-output-vocab", dest="output_vocab_size", type=int, help="Output Vocabulary size. Defaults to target vocabulary size.")
...
# File paths
if args.shuffle:
	output_mmap_path = args.output_dir_path + "/train.idx.shuf.mmap"
	output_idx_path = args.output_dir_path + "/train.idx.shuf.txt"
	output_ngrams_path = args.output_dir_path + "/train.shuf.txt"
else:
	output_mmap_path = args.output_dir_path + "/train.idx.mmap"
	output_idx_path = args.output_dir_path + "/train.idx.txt"
	output_ngrams_path = args.output_dir_path + "/train.txt"
...
output_word_to_id = process_vocab(args.trg_input_path, args.output_vocab_size, output_prefix+'.vocab') # Word to index dictionary of output vocab
...
with open(tmp_path, 'r') as data:
	fp = np.memmap(output_mmap_path, dtype='int32', mode='w+', shape=(nsamples + 1, ngram_size))
	fp[0,0] = nsamples												# number of samples
	fp[0,1] = ngram_size											# n-gram size
	fp[0,2] = ovocab												# number of word types (MLP classes)
	counter = 1
	for line in data:
		tokens = line.split()
		fp[counter] = tokens
