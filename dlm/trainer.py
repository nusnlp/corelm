...
def train(classifier, criterion, args, trainset, devset, testset=None):
	...
	lr_tuner = LRTuner(low=0.01, high=1, inc=0.01)
...
class Hook:
	...
	def evaluate(self, curr_iter):
		...
		L.info(('DEV =>  Error=%.2f%%, PPL=' + U.BColors.BYELLOW + '%.2f' + U.BColors.ENDC + ' (best=' + U.BColors.BRED + '%.2f @ %i' + U.BColors.ENDC + '), Denom=%.3f, %im')
			% (dev_error * 100., dev_perplexity, self.best_dev_perplexity, self.best_iter, denominator, rem_time / 60))
		if self.test_eval:
